"""
This is the conftest for selenium_tests with logging into log database
...
from opencart_tests_with_database.models.page_objects.page_objects import ProductsPage, LoginPage
from selenium.webdriver.support.events import EventFiringWebDriver, AbstractEventListener
from browsermobproxy import Server
from opencart_tests_with_database.models.page import db, cursor
from opencart_tests_with_database.models.page import loglevel
# import of urlparse for different Python versions
try:
...
for proc in psutil.process_iter():
    ...
    if "browsermob-proxy" in proc.name():
        proc.kill()
...
PROXY_PATH = "/home/asayapova/PycharmProjects/soft/browsermob-proxy-2.1.4-bin/bin/browsermob-proxy"

server = Server(PROXY_PATH, {"port": 9090})
server.start()
...
class MyListener(AbstractEventListener):
    def before_navigate_to(self, url, driver):
        message = "".join(["Open page by URL:", url])
        print(message)
        cursor.execute("""insert into log.main (datetime, loglevel, info) values
                        ('{}', '{}', '{}')""".format(datetime.datetime.now(), loglevel, message))
        db.commit()
    def after_navigate_to(self, url, driver):
        message = "".join([url, "page opened"])
        print(message)
        cursor.execute("""insert into log.main (datetime, loglevel, info) values
                        ('{}', '{}', '{}')""".format(datetime.datetime.now(), loglevel, message))
        db.commit()
    ...
    def on_exception(self, exception, driver):
        ...
        print(exception)
        cursor.execute("""insert into log.main (datetime, loglevel, info) values
                        ('{}', '{}', '{}')""".format(datetime.datetime.now(), loglevel, exception))
        db.commit()

@pytest.fixture(scope="session")
def close_log_db_connestion(request):
    def close():
        cursor.close()
        db.close()
...
def chosen_browser(request):
    ...
    if parameter == "chrome":
        ...
        options = IeOptions()
        url = urlparse(proxy.proxy).path
        options.add_argument('--set-proxy=%s' % url)
        options.add_argument('--proxy-server=%s' % url)
        options.add_argument("--start-fullscreen")
        options.add_argument("--headless")
